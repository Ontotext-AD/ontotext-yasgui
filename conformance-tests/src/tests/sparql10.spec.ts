...
const SUITE: Suite = {
  name: 'SPARQL 1.0',
  dir: path.resolve(__dirname, '..', '..', '..', 'test-files', 'sparql10')
};
/**
 ...
 *
 * - Incomplete queries: queries that are syntactically valid prefixes but
 *   missing required clauses (e.g. SELECT with no WHERE).
 */
const SKIPPED_NEGATIVE_TESTS = new Set([
  // Incomplete queries — parser says OK (no error so far) but the query is not complete
...
describe(`W3C SPARQL 1.0 Syntax Conformance Test`, () => {
